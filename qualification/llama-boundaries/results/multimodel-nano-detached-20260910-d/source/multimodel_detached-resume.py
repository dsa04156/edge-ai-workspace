...
def main():
    ...
    a=p.parse_args(); root=a.output.resolve(); root.mkdir(exist_ok=a.resume,parents=True)
    original_command=m.command
    def resilient_command(args, timeout=30):
        # Only read queries and idempotent evidence copies may be repeated.
        # Never repeat a workload launch, model lifecycle or service mutation.
        joined=" ".join(args)
        retryable=not re.search(r"\b(nohup|setsid|patch|kill|mkdir|rm)\b|subprocess\.Popen",joined)
        for attempt in range(5 if retryable else 1):
            try:return original_command(args,timeout)
            except (subprocess.CalledProcessError,subprocess.TimeoutExpired) as exc:
                with (root/"control-errors.jsonl").open("a") as log:
                    log.write(json.dumps({"at":time.time(),"attempt":attempt,"retryable_read":retryable,
                        "error":str(exc),"output":str(getattr(exc,"output",""))[:1000]})+"\n")
                if not retryable or attempt==4:raise
                time.sleep(min(2**(attempt+1),15))
    m.command=resilient_command
    ...
    if not a.resume:
        ...
        previous=json.loads((root/"nodes-before.json").read_text())[a.node]
        assert previous["uid"]==n["uid"]
    source=root/"source";source.mkdir(exist_ok=a.resume)
    ...
    (source/("multimodel_detached-resume.py" if a.resume else "multimodel_detached.py")).write_text(Path(__file__).read_text())
    (root/("source-hashes-resume.json" if a.resume else "source-hashes.json")).write_text(json.dumps({f.name:bench.digest(f) for f in source.iterdir()},indent=2))
    paused_here=False
    try:
        for block,slugs in enumerate(plan["order"]):
            for slug in slugs:
                old=a.parent/a.node/(slug+f"-b{block}")
                own=root/a.node/(slug+f"-b{block}")
                if (own/"workload/complete.json").exists() and json.loads((own/"cleanup.json").read_text()).get("port_released"):
                    continue
