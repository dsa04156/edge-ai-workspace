...
이 overlay는 `etri-ser0001-cg0msb`의 inference 후보 endpoint와
`AugmentationResource` 관측 계약만 준비한다. 상위 `k8s/kustomization.yaml`에는 포함하지
않으므로 현재 Argo CD 운영 경로에 자동 반영되지 않는다.
...
현재 online baseline은 저장된 학습 artifact가 아니라 입력으로 warm-up되는 모델이다.
...
Docker daemon 없이 `scripts/build-server1-oci.sh`로 생성한 linux/amd64 image를
`sha256:9df19090241fcc223bcd91e435f6ed0278444daf85c6fec74b8e7a4e9eb3a5be`로 고정했다.
스크립트는 `crane` archive checksum과 Python 3.11 amd64 base digest를 함께 고정한다.
...
- 모델 `ready`, 입력 `fresh`, server1 CPU·Memory 여유
- `AugmentationResource.status.phase=Available`, `endpointReady=true`, `freeInstances>0`

2026-08-13 선택적 overlay를 수동 적용해 다음 observed-only 증거를 확인했다.
