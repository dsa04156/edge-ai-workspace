...
class LocalDataClient:
    ...
    def _validated_samples(
        ...
        try:
            ...
            raise LocalDataResponseError("Local Data response is not valid JSON") from exc
        ...
        if payload.get("resourceName") != source.resource_name:
            raise LocalDataResponseError("Local Data resourceName does not match request")
        ...
        if (
            ...
            raise LocalDataResponseError("Local Data retention.maxSamples must be positive")
        ...
        for index, sample in enumerate(samples):
            ...
            if isinstance(origin, bool) or not isinstance(origin, int) or origin <= 0:
                raise LocalDataResponseError(f"samples[{index}].origin must be positive")
            if value_type != "Int32":
                raise LocalDataResponseError(f"samples[{index}].valueType must be Int32")
            if isinstance(value, bool) or not isinstance(value, int):
                raise LocalDataResponseError(f"samples[{index}].value must be an integer")
            if previous_origin is not None and origin < previous_origin:
                raise LocalDataResponseError("Local Data samples must be origin ordered")
            validated.append(
