...
- `edgeAI/scripts/generate_devices.py`
  - `status_keys` 를 따로 두고, 해당 key 만 `reportToCloud: true` 로 생성
- `edgeAI/temp-device.yaml`
  - `temperature`: `reportToCloud: false`
  - `sampling_interval`: `reportToCloud: true`
...
- `env`
  ...
  - status: `sampling_interval`
- `vib`
  - telemetry: `vibration`
  - status: `alarm`, `sampling_interval`
- `act`
  - status: `power`, `mode`, `sampling_interval`
...
- `DeviceStatus` 보고는 5초 flush + changed-only
