...
- 전체 PoC 기준 총 30개 Device manifest 구성
...
6. raw telemetry는 MQTT consumer 또는 DB consumer 경로로 별도 저장한다.
7. 디바이스-서비스 바인딩과 대시보드 표시 구조를 연결한다.
8. 자동 발견/자동 등록 controller는 데모 안정화 이후 후속 과제로 설계한다.
