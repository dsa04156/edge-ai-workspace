...
핵심 판단:
- KubeEdge 원래 방식에 맞춰 `DeviceStatus`는 디바이스별 최신 reported snapshot으로 사용한다.
- `DeviceStatus`는 고속 시계열 저장소가 아니므로 저주기/스로틀링된 최신값만 올린다.
- InfluxDB는 raw telemetry 이력 저장소로 사용한다.
- 대시보드는 `DeviceStatus`의 최신 snapshot을 우선 표시하되, `lastOnlineTime` 또는 reported timestamp가 오래되면 stale로 표시한다.
...
현재 정책:
1. Device `status.reportToCloud`는 켠다.
2. Device state 보고 주기는 최소 30초로 둔다.
3. `DeviceStatus.status.twins.reported`에는 manifest property의 최신 snapshot을 올린다.
4. mapper는 property별 최소 30초 간격으로 twin report를 throttle한다.
5. 값이 같아도 timestamp 갱신을 위해 30초 주기에는 다시 report할 수 있다.
6. InfluxDB에는 raw telemetry history를 저장한다.

DeviceStatus property 의미:
- `temperature`, `humidity`, `vibration`: 최신 센서 snapshot. 고속 이력은 InfluxDB 기준.
...
현재 manifest 정책:
- 대시보드에서 최신 snapshot으로 볼 property: `reportToCloud: true`
- 이력/그래프가 필요한 raw telemetry: InfluxDB `pushMethod.dbMethod.influxdb2` 병행
- Device `status.reportToCloud: true`
- Device `status.reportCycle: 30000`

주의:
- `act-device-06 power:on`처럼 `DeviceStatus`에 값이 있어도 `lastOnlineTime`이 오래됐으면 현재값이 아니라 마지막 reported 값이다.
...
- `DeviceStatus`는 최신 reported snapshot으로 사용한다.
- 실제 live 여부는 `DeviceStatus.lastOnlineTime` 또는 reported timestamp가 fresh한지 먼저 판단한다.
- InfluxDB 최신 수신 시각은 이력/그래프와 fallback freshness 판단에 사용한다.
...
MQTT telemetry
  -> mqttvirtual mapper
  -> DeviceStatus latest reported snapshot
  -> InfluxDB device_telemetry bucket for history
  -> state-aggregator KubeEdge DeviceStatus + InfluxDB query
  -> device별 live/stale 및 telemetry_last_seen 계산
  -> dashboard 표시
```

현재 `state-aggregator` 판단 기준:
- `healthy`: `DeviceStatus`의 state/twin snapshot이 freshness 기준 이내로 확인됨. DeviceStatus가 없을 때는 InfluxDB 최신 telemetry를 fallback으로 사용.
- `degraded`: DeviceStatus 또는 InfluxDB 최신값이 freshness 기준을 넘었거나, 노드와 mapper는 살아 있지만 최근 telemetry가 없음.
...
3. 할당 노드가 `unavailable`이면 디바이스도 `unavailable`이다.
4. `DeviceStatus.lastOnlineTime`이 있고 `TELEMETRY_FRESH_SECONDS`보다 오래됐으면 `degraded`로 본다.
5. `DeviceStatus.status`, `phase`, `state`, `connection`, `connected`, `health` 중 명시 상태값이 있으면 우선 적용한다.
6. 명시 상태값이 `online`, `connected`, `healthy`, `active`, `true`이면 `healthy`다.
7. 명시 상태값이 `offline`, `disconnected`, `failed`, `unavailable`, `false`이면 `unavailable`이다.
8. 할당 노드가 `degraded`이면 디바이스도 `degraded`다.
9. reported twin 값이 하나라도 있으면 최신 twin snapshot이 있다고 보고 `healthy`다.
10. InfluxDB 최신 telemetry가 있으면 freshness 기준 이내는 `healthy`, 오래됐으면 `degraded`다.
11. 노드 미할당이면 `unavailable`이다.
12. `mqttvirtual` 디바이스인데 해당 노드에 mapper Pod가 Running이 아니면 `unavailable`이다.
13. 노드와 mapper는 살아 있고 telemetry 대상인데 InfluxDB에 최근 값이 없으면 `degraded`다.
14. `reportToCloud: false`이거나 live status가 전혀 없으면 `unavailable`이다.
15. telemetry-to-cloud가 꺼져 있으면 `degraded`다.
16. 그 외에는 등록은 됐지만 live 상태를 모르는 것으로 보고 `degraded`다.

상태 판단에 쓰는 실제 필드:
- `Device.spec.nodeName`: 어느 노드에 할당됐는지 확인한다.
- `Device.spec.protocol.protocolName`: `mqttvirtual` mapper 확인 여부를 결정한다.
- `Device.spec.properties[].reportToCloud` 또는 `pushMethod`: telemetry 대상인지 판단한다.
- `DeviceStatus.status.lastOnlineTime`: DeviceStatus snapshot freshness를 판단한다.
- `DeviceStatus.status.twins` 또는 `twin`: reported twin 값 존재 여부를 판단한다.
- InfluxDB `device_telemetry`: DeviceStatus가 없거나 보조 판단이 필요할 때 최신 telemetry 수신 시각을 본다.
...
검증 기준:
- `temperature`, `humidity`, `vibration`은 DeviceStatus에 최신 snapshot으로 올라갈 수 있지만, 고속 이력/그래프 기준은 InfluxDB다.
- `health`, `severity`, `alarm_latched`, `power`, `mode`, `sampling_interval`은 운영 상태 판단에 사용한다.
- Device `status.reportToCloud`는 true, `reportCycle`은 최소 30000ms로 유지한다.
- mapper 로그에 DeviceStates rate limit 오류가 반복되지 않는다.
- 서비스 데모에서 raw telemetry는 별도 data plane으로 처리된다.
- dashboard의 `healthy`는 Device CR 존재 여부가 아니라 DeviceStatus freshness와 InfluxDB `last_seen`을 함께 보고 판단된다.
