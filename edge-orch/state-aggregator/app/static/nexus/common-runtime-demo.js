...
function actions(data,uid,current,pending,manual=false){
 ...
 return `<div class="runtime-demo-actions"><small>${E(item.label)}</small><div class="runtime-action-buttons">${button('single','demo-single')}${button(manual?'load':'round-trip','demo-round')}</div><small>최대 ${E(item.maxRequests)}건 · 동시 ${E(item.concurrency)}건${item.retryAfterSeconds?' · 재시험까지 '+E(item.retryAfterSeconds)+'초':''}</small></div>`;
}
