...
const api={actions,panel,newId,setup,refresh,renderActions:(uid,current,manual=false)=>actions(data,uid,current&&!error&&Date.now()/1000-(data?.observedAt||0)<15,intents.has(uid)||sending.has(uid),manual),renderPanel:()=>panel(data,error,[...intents.values()])+(actionError?`<p class="runtime-warning" role="alert">시험 접수: ${E(actionError)}</p>`:'')};
