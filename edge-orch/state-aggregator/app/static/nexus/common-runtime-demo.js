...
const controlLabels={Running:'서비스 실행 중',Starting:'서비스 시작 중 · 실행 준비',Stopping:'서비스 중지 중 · 요청 마무리·자원 정리',Stopped:'서비스 중지됨'};
function serviceActions(value,uid,current,pending=false){
 ...
 return `<section class="runtime-service-controls" aria-label="서비스 실행 제어"><div class="runtime-control-status" role="status">${current&&(request||starting||stopping)?'<span class="runtime-spinner" aria-hidden="true"></span>':''}<strong>${!current?'서비스 상태 확인 불가':request?(request==='start'?'서비스 시작 접수 중…':'서비스 중지 접수 중…'):E(controlLabels[state.phase])}</strong></div><div class="runtime-service-buttons"><button id="service-start-${E(uid)}" class="button primary" data-service-action="start" data-service-uid="${E(uid)}" ${!current||!state.canStart||request?'disabled':''} aria-busy="${Boolean(request==='start'||current&&starting)}">${request==='start'||current&&starting?'시작 중…':'서비스 실행'}</button><button id="service-stop-${E(uid)}" class="button runtime-service-stop" data-service-action="stop" data-service-uid="${E(uid)}" ${!current||!state.canStop||request?'disabled':''} aria-busy="${Boolean(request==='stop'||current&&stopping)}">${request==='stop'||current&&stopping?'중지 중…':'서비스 중지'}</button></div><p>실행: 서비스 준비 후 요청 대기<br>중지: 이 서비스의 부하 제거 → 진행 요청 마무리 → 실행 자원 정리</p>${controlErrors.has(uid)?`<p class="runtime-warning" role="alert">${E(controlErrors.get(uid))}</p>`:''}<button id="demo-single-${E(uid)}" class="button runtime-single" data-demo-start="${E(uid)}" data-demo-mode="single" ${!current||!item.available||run||pending||request?'disabled':''}>${pendingActions.get(uid)?.mode==='single'?'요청 접수 중…':'시험 요청 1건'}</button></section>`;
}
function serviceLoadActions(value,uid,current,pending=false){
 const item=value?.items?.find(x=>x.uid===uid);if(!item)return '';
 const run=(value.runs||[]).find(r=>r.uid===uid&&active(r)),request=pendingActions.get(uid);
 const stopping=Boolean(run?.phase==='Stopping'||request?.stop);
 const busy=Boolean(current&&run?.mode==='service-load'&&active(run));
 return `<section class="runtime-service-load" aria-label="선택 서비스 부하 제어"><h3>서비스 부하</h3><div class="runtime-service-buttons"><button id="service-load-${E(uid)}" class="button primary" data-demo-start="${E(uid)}" data-demo-mode="service-load" ${!current||!item.available||run||pending||controlPending.has(uid)?'disabled':''} aria-busy="${Boolean(request&&!request.stop||current&&busy&&!stopping)}">${request&&!request.stop?'시작 접수 중…':busy?'서비스 부하 중…':'서비스 부하 시작'}</button><button id="service-unload-${E(uid)}" class="button runtime-stop" ${run?`data-demo-stop="${E(run.id)}" data-demo-uid="${E(uid)}" data-demo-name="${E(item.name)}"`:''} ${!run||stopping?'disabled':''} aria-busy="${stopping}">${stopping?'부하 제거 중…':'부하 제거'}</button></div><p>처리 노드가 바뀌어도 같은 서비스에 요청을 계속 보냅니다. 부하 제거 시 새 요청을 멈추고 진행 요청을 마무리합니다.</p><small>${item.testConfigured===false?'시험 입력 미등록 · 서비스 계약에 입력을 연결해야 부하를 줄 수 있습니다.':!item.available&&!run?'서비스 준비·시험 가능 조건 확인 필요':'동시 '+E(item.concurrency)+'건 · 제거할 때까지 유지 · 화면을 바꿔도 계속 실행'}</small>${!current?'<p class="runtime-warning">부하 관측 확인 불가 · 마지막 실행 ID의 중단을 요청할 수 있습니다.</p>':''}</section>`;
}
function nodeActions(value,uid,node,label,current,pending=false){
 ...
 const follows=r=>r.followsService||r.mode==='service-load';
 const origin=r=>r.targetNode||r.startNode||r.currentNode;
 const atNode=r=>origin(r)===node;
 const nodeLabel=n=>item?.nodes?.find(x=>x.node===n)?.label||n;
 const rows=(value?.runs||[]).filter(r=>r.uid===uid&&atNode(r)).sort((a,b)=>b.createdAt-a.createdAt);
 const incoming=(value?.runs||[]).find(r=>r.uid===uid&&active(r)&&follows(r)&&r.currentNode===node&&!atNode(r));
...
function setup(callback){draw=callback;try{for(const i of JSON.parse(root.localStorage.getItem('nexus-runtime-demo-intents-v1')||'[]').slice(0,20))if(i&&typeof i.uid==='string'&&typeof i.id==='string'&&typeof i.name==='string'&&['single','round-trip','load','node-load','service-load'].includes(i.mode))intents.set(i.uid,i);}catch(_){}
...
function scopedData(value,uid){return !value||!uid?value:{...value,items:value.items.filter(x=>x.uid===uid),runs:value.runs.filter(x=>x.uid===uid)};}
const api={actions,serviceActions,serviceLoadActions,scopedData,nodeActions,panel,
 renderServiceLoadActions:(uid,current)=>serviceLoadActions(data,uid,current&&context(uid).current,intents.has(uid)||sending.has(uid)),
 renderNodeActions:(uid,node,label,current)=>nodeActions(data,uid,node,label,current&&context(uid).current,intents.has(uid)||sending.has(uid)),newId,setup,refresh,context,runStatus,renderActions:(uid,current,manual=false)=>actions(data,uid,current&&context(uid).current,intents.has(uid)||sending.has(uid),manual),renderPanel:uid=>panel(scopedData(data,uid),error,[...intents.values()].filter(i=>!uid||i.uid===uid),serverNow())+(actionError&&(!uid||actionError.uid===uid)?`<p class="runtime-warning" role="alert">시험 접수: ${E(actionError.message)}</p>`:'')};
