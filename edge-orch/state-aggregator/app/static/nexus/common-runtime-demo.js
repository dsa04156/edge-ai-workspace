...
const labels={Running:'시험 중',Stopping:'남은 요청 마무리',Stopped:'사용자 중단',Completed:'검증 통과',Incomplete:'검증 미완료',Interrupted:'중단 · 결과 확인 필요'};
...
function actions(data,uid,current,pending){
 const item=data?.items?.find(x=>x.uid===uid);if(!item)return '';
 const busy=data.runs.some(r=>r.uid===uid&&active(r));
 const disabled=!current||!item.available||busy||pending;
 return `<div class="runtime-demo-actions"><small>${E(item.label)}</small><button id="demo-single-${E(uid)}" class="button" data-demo-start="${E(uid)}" data-demo-mode="single" ${disabled?'disabled':''}>시험 요청 1건</button><button id="demo-round-${E(uid)}" class="button" data-demo-start="${E(uid)}" data-demo-mode="round-trip" ${disabled?'disabled':''}>왕복 시험</button><small>최대 ${E(item.maxRequests)}건 · 동시 ${E(item.concurrency)}건${item.retryAfterSeconds?' · 재시험까지 '+E(item.retryAfterSeconds)+'초':''}</small></div>`;
}
...
async function submit(i,stop=false){if(sending.has(i.uid))return;sending.add(i.uid);actionError=null;draw();const c=new AbortController(),timer=setTimeout(()=>c.abort(),5000);try{
 ...
 if(!r.ok){if(r.status<500&&!stop){intents.delete(i.uid);persist();}actionError=String(result.detail||'demo_request_failed');return;}
 if(!stop){intents.delete(i.uid);persist();}
 if(data){data.runs=[result,...data.runs.filter(x=>!(x.uid===result.uid&&x.id===result.id))];}
 error=null;
 }catch(_){actionError=stop?'중단 접수 결과 확인 필요':'시작 접수 결과 확인 필요';}
 finally{clearTimeout(timer);sending.delete(i.uid);draw();refresh();}}
function setup(callback){draw=callback;try{for(const i of JSON.parse(root.localStorage.getItem('nexus-runtime-demo-intents-v1')||'[]').slice(0,20))if(i&&typeof i.uid==='string'&&typeof i.id==='string'&&typeof i.name==='string'&&['single','round-trip'].includes(i.mode))intents.set(i.uid,i);}catch(_){}
...
const api={actions,panel,newId,setup,refresh,renderActions:(uid,current)=>actions(data,uid,current&&!error&&Date.now()/1000-(data?.observedAt||0)<15,intents.has(uid)||sending.has(uid)),renderPanel:()=>panel(data,error,[...intents.values()])+(actionError?`<p class="runtime-warning" role="alert">시험 접수: ${E(actionError)}</p>`:'')};
