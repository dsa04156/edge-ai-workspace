...
function nodeActions(value,uid,node,label,current,pending=false){
 ...
 const atNode=r=>follows(r)?(r.currentNode||r.startNode||r.targetNode)===node:r.targetNode===node;
 const rows=(value?.runs||[]).filter(r=>r.uid===uid&&atNode(r)).sort((a,b)=>b.createdAt-a.createdAt);
 const moved=(value?.runs||[]).find(r=>r.uid===uid&&active(r)&&follows(r)&&r.targetNode===node&&!atNode(r));
 ...
 const note=!current?'부하 상태 확인 불가':stopping?'새 요청 중단 · 진행 요청 마무리 중':run?(run.startNode&&run.startNode!==node?'부하 유지 · '+run.startNode+'에서 이동':'제거할 때까지 유지 · 서비스 이동 후에도 계속'):ownRequest?'부하 시작 접수 중…':moved?'부하는 '+moved.currentNode+'에서 계속 실행 중':last?.reason==='node_route_changed'?'실행 노드 이동 · 이 노드 부하 종료':last?.phase==='Stopped'?'이 노드 부하 제거 완료':last?.phase==='Completed'?'이 노드 부하 완료':target?.retryAfterSeconds?'재시험까지 '+target.retryAfterSeconds+'초':target?.currentRoute?'부하 대기 · 서비스 실행 유지':'서비스가 이 노드에서 실행되면 사용';
