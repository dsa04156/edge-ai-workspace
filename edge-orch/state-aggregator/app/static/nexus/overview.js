...
function model(entries,now=Date.now()){
 ...
 const usage=resources.map(r=>{const node=items('nodes').find(n=>n.hostname===r.node);const failed=current('nodes')&&fresh(node?.collected_at,now)&&(node.node_health==='unavailable'||node.raw_metrics?.up===0);return {name:r.node,...Object.fromEntries(['cpuRatio','memoryRatio','gpuRatio'].map(k=>[k,!failed&&current('resources')&&fresh(r.utilization?.observedAt,now)&&number(r.utilization[k])&&r.utilization[k]>=0&&r.utilization[k]<=1?r.utilization[k]*100:null]))};});
...
function render(entries){
 ...
 const eventRows=UX.groupEvents(m.events).slice(0,4).map(e=>`<li><span class="ov-event-marker"></span><div><button data-live-service="${E(e.service_id)}">${E(e.type==='Scheduling Decision'?'배치 검토':e.type)} · ${E(label(e.state))}${e.count>1?' · '+e.count+'회':''}</button><p title="${E((e.reasons||[]).map(r=>reasons[r]||r).join(' · '))}">${E((e.reasons||[]).map(r=>reasons[r]||r).join(' · ')||e.service_id)}</p></div><time>${E(time(e.timestamp))}</time></li>`).join('');
