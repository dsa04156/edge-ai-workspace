...
function runtimeMap(s,m,now){
 const nodes=mapNodes(s,m,now),height=Math.max(nodes.length,1)*132-12;
 const chosen=nodes.find(t=>t.node===selectedNodes.get(s.uid));
...
function augmentationView(s,now,current){
 ...
 const ctx=root.NexusRuntimeDemo?.context(s.uid,now);if(ctx)ctx.detailsOpen=expanded.has("run-"+s.uid);
...
const api={augmentationView,serviceMotion,mapNodes,runtimeMap,renderState,targetView,latencyView,activate,setup,render:()=>renderState(entry,Date.now()/1000,expanded)+`<details class="runtime-diagnostics" data-runtime-details="history" ${expanded.has('history')?'open':''}><summary>시험 실행 이력 · 응답 상세</summary>${root.NexusRuntimeDemo?.renderPanel()||''}</details>`};
