...
function renderWorkspace(value,uid,now=observationNow(value)){
 ...
 return picker+operationsView(service,now,current)+placement+(service.managed===false?'':(root.NexusRequestHistory?.render(service.uid,service.name)||'')+renderState(scoped,now,expanded,true,true)+`<details class="runtime-diagnostics" data-runtime-details="history" ${expanded.has('history')?'open':''}><summary>${E(service.name)} · 부하 실행 이력과 응답</summary>${root.NexusRuntimeDemo?.renderPanel(service.uid)||''}</details>`);
}
function render(){
