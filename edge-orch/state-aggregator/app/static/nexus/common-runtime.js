...
const api={returnView,observationNow,nodeMetrics,nodeMetricsView,augmentationView,serviceMotion,mapNodes,runtimeMap,renderState,targetView,latencyView,activate,setup,render:()=>renderState(entry,observationNow(entry),expanded)+`<details class="runtime-diagnostics" data-runtime-details="history" ${expanded.has('history')?'open':''}><summary>시험 실행 이력 · 응답 상세</summary>${root.NexusRuntimeDemo?.renderPanel()||''}</details>`};
