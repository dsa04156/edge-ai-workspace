...
function mapNodes(s,m,now){
 const nodes=new Map();
 for(const t of [...(s.contractSummary?.candidateNodes||[]).map(node=>({node})),...(s.augmentationStages||[]),...(m.ok&&s.aiInference?s.eligibleCandidates||[]:[]),s.active,s.target,...(s.retiring||[]),m.p])if(t?.node)nodes.set(t.node,{...(nodes.get(t.node)||{}),...t});
...
function returnView(s,now,current){
 if(s.placementMode==='preferred')return '';
...
function renderWorkspace(value,uid,now=observationNow(value)){
 ...
 const resources=root.NexusLive?.store?.entries?.resources;
 const placement=root.NexusPlacementOverview?.render(service,{now,current,nodes:nodeEntry.data||[],hardware:n=>nodeMetrics(n,now),resources:resources?.data||[],resourcesCurrent:Boolean(root.NexusData?.isCurrent(resources)),resourcesReceivedAt:resources?.receivedAt,selectedNode:selectedNodes.get(service.uid)})||'';
 return picker+operationsView(service,now,current)+`<details class="runtime-diagnostics" data-runtime-details="placement-overview" ${expanded.has('placement-overview')?'open':''}><summary>전체 노드 · 등록 조건과 제외 근거</summary>${placement}</details>`+(service.managed===false?'':(root.NexusRequestHistory?.render(service.uid,service.name)||'')+renderState(scoped,now,expanded,true,true)+`<details class="runtime-diagnostics" data-runtime-details="history" ${expanded.has('history')?'open':''}><summary>${E(service.name)} · 부하 실행 이력과 응답</summary>${root.NexusRuntimeDemo?.renderPanel(service.uid)||''}</details>`);
}
function render(){
