...
function serviceMotion(s,now,current){
 ...
 let title=!ok?'관측 연결 확인 중':s.target?'모델 준비 중':(s.retiring||[]).length?'이전 요청 마무리 중':p?'추천 도착 · 승인 대기':busy?'AI 추론 실행 중':s.serving?'요청 대기 중':phases[s.phase]||'서비스 준비 확인';
 return {ok,load,l,p,busy,title};
}
function mapNodes(s,m,now){
 const nodes=new Map();
 for(const t of [...(m.ok?s.eligibleCandidates||[]:[]),s.active,s.target,...(s.retiring||[]),m.p])if(t?.node)nodes.set(t.node,{...(nodes.get(t.node)||{}),...t});
 return [...nodes.values()].sort((a,b)=>(a.role==='server')-(b.role==='server')||a.node.localeCompare(b.node)).map(t=>{
  ...
  const state=!m.ok?'unknown':preparing?'preparing':retiring?'retiring':current?(s.serving?'active':'unknown'):recommended?'recommended':'candidate';
  const title={unknown:'현재 상태 확인 불가',preparing:'모델 준비 중',retiring:'남은 요청 마무리',active:m.busy?'추론 실행 중':'요청 대기 중',recommended:'증강 추천 · 승인 대기',candidate:'배치 후보 · 모델 미관측'}[state];
...
function nodeMetrics(node,now,entry=nodeEntry){
 ...
 const current=Boolean(!entry.error&&snapshot&&now-entry.receivedAt>=0&&now-entry.receivedAt<60&&Number.isFinite(at)&&now-at>=-5&&now-at<60&&snapshot.raw_metrics?.up===1&&snapshot.node_health!=='unavailable');
 const raw=current?snapshot.raw_metrics:{};
 const ratio=v=>typeof v==='number'&&Number.isFinite(v)&&v>=0&&v<=1?v:null;
 return {current,at,health:current?snapshot.node_health:null,cpu:ratio(raw.cpu_utilization),memory:ratio(raw.memory_usage_ratio),gpu:ratio(raw.gpu_utilization),gpuMemory:ratio(raw.gpu_memory_usage_ratio),gpuUsed:raw.gpu_memory_used_mib??null,gpuTotal:raw.gpu_memory_total_mib??null,gpuTemperature:raw.gpu_temperature_celsius??null};
}
function nodeMetricsView(node,now,entry=nodeEntry){
 const m=nodeMetrics(node,now,entry),percent=v=>(v*100).toFixed(1)+'%';
 return `<span class="runtime-node-metrics" aria-label="노드 전체 CPU 메모리 GPU 사용률">${[['CPU',m.cpu],['메모리',m.memory],['GPU',m.gpu]].map(([label,v])=>`<span class="runtime-hardware-metric"><span>${label}</span><b>${v==null?(m.current?'미수집':'—'):percent(v)}</b>${v==null?'<i class="missing"></i>':`<i role="meter" aria-label="노드 ${label} 사용률" aria-valuemin="0" aria-valuemax="100" aria-valuenow="${(v*100).toFixed(1)}"><i style="transform:scaleX(${v})"></i></i>`}</span>`).join('')}</span><small class="runtime-node-observed">${m.current?`노드 ${E({healthy:'정상',degraded:'주의'}[m.health]||'상태 확인')} · ${E(new Date(m.at*1000).toLocaleTimeString('ko-KR',{hour12:false}))}`:'노드 지표 관측 불가 · 최신 수집 확인 필요'}</small>`;
}
async function refreshNodes(){
 if(nodeFetch)return nodeFetch;
 nodeFetch=(async()=>{try{const r=await root.fetch('/state/nodes',{cache:'no-store',signal:AbortSignal.timeout(4500)});if(!r.ok)throw new Error('node_source_unavailable');const data=await r.json();if(!Array.isArray(data))throw new Error('invalid_nodes');nodeEntry={data,error:null,receivedAt:Date.now()/1000};}catch(_){nodeEntry.error='node_source_unavailable';}finally{nodeFetch=null;if(enabled)draw();}})();return nodeFetch;
}
function runtimeMap(s,m,now){
 const nodes=mapNodes(s,m,now),height=Math.max(nodes.length,1)*164-12;
 ...
 return `<section class="runtime-map" aria-label="AI 서비스 노드 실행 지도"><div class="runtime-map-heading"><h3>요청 경로와 실행 노드</h3><span>노드를 눌러 상세 확인</span></div><div class="runtime-map-canvas"><div class="runtime-map-source"><span class="runtime-source-icon" aria-hidden="true">↗</span><strong>AI 추론 요청</strong><small>${m.ok?`${num(m.load?.pending)}건 대기 · ${num(m.load?.inFlight)}건 처리 중`:'현재 요청 관측 불가'}</small></div><svg class="runtime-map-links" viewBox="0 0 80 ${height}" preserveAspectRatio="none" aria-hidden="true">${nodes.map((t,i)=>`<path class="${t.state} ${t.flow?'flowing':''}" d="M0 ${height/2} C40 ${height/2} 40 ${i*164+76} 80 ${i*164+76}"/>`).join('')}</svg><div class="runtime-map-nodes">${nodes.map(t=>`<button type="button" id="map-${E(s.uid)}-${E(t.node)}" class="runtime-map-node ${t.state}" data-runtime-node="${E(t.node)}" data-runtime-uid="${E(s.uid)}" aria-pressed="${chosen?.node===t.node}"><span class="runtime-node-icon">${icon}</span><span class="runtime-node-copy"><small>${E(t.role==='edge'?'현장 엣지':t.role==='server'?'서버':'실행 노드')}</small><strong>${E(t.node)}</strong><span class="runtime-node-state">${t.state==='preparing'||t.state==='retiring'||t.flow?'<span class="runtime-spinner" aria-hidden="true"></span>':'<span class="runtime-status-dot" aria-hidden="true"></span>'}${E(t.title)}</span><small>${t.health?.modelVramMiB!=null?'모델 '+num(t.health.modelVramMiB)+' MiB':t.state==='recommended'?'클릭 승인 후 모델 준비':t.state==='candidate'?'실행 준비 여부는 별도 확인':'모델 메모리 관측 없음'}</small>${nodeMetricsView(t.node,now)}</span></button>`).join('')||'<p class="empty">실행 노드 관측 대기</p>'}</div></div><div class="runtime-map-legend"><span><i class="active"></i>현재 요청 경로</span><span><i class="recommended"></i>증강 추천</span><span><i class="preparing"></i>모델 준비</span><span>흐르는 선: 실제 처리·대기 요청 · CPU·메모리·GPU: 노드 전체 실측</span></div>${chosen?`<div class="runtime-map-detail" role="region" aria-label="선택 노드 상세"><strong>${E(chosen.node)}</strong><span>${E(chosen.variant)} · ${E(chosen.title)}</span><span>동시 처리 한도 ${num(chosen.capacity)}건 · 모델 ${num(chosen.health?.modelVramMiB)} MiB</span><span>노드 GPU 메모리 ${num(hardware.gpuUsed)} / ${num(hardware.gpuTotal)} MiB · GPU 온도 ${num(hardware.gpuTemperature)} °C</span></div>`:''}</section>`;
}
function augmentationView(s,now,current){
 ...
 return `<article class="panel augmentation-panel" data-augmentation-service="${E(s.uid)}"><header class="augmentation-header"><div><p class="eyebrow">AI 서비스 실행 지도</p><h2>${E(s.name)}</h2></div><div class="runtime-service-state ${ok?'':'unknown'}" role="status">${ok&&(s.target||m.busy||(s.retiring||[]).length)?'<span class="runtime-spinner" aria-hidden="true"></span>':'<span class="runtime-status-dot" aria-hidden="true"></span>'}<strong>${E(m.title)}</strong></div><button class="button runtime-refresh" data-runtime-refresh aria-label="AI 실행 상태 새로고침">↻</button></header><div class="augmentation-workspace">${runtimeMap(s,m,now)}<aside class="augmentation-controls" aria-label="부하 시험과 증강 승인"><h3>부하 시험 · 실행 제어</h3>${root.NexusRuntimeDemo?.renderActions(s.uid,ok,s.approvalRequired)||(ctx?.loading?'<p class="runtime-loading"><span class="runtime-spinner" aria-hidden="true"></span>시험 설정 조회 중…</p>':`<p>${ctx?.current?'등록된 부하 시험이 없습니다.':'시험 설정 관측을 확인할 수 없습니다.'}</p>`)}${ctx?root.NexusRuntimeDemo.runStatus(ctx,now):''}<div class="augmentation-approval"><div><strong>${p?'증강 추천 도착':s.target?'모델 준비 중':'증강 승인'}</strong><p>${!ok?'최신 관측을 확인한 후 승인할 수 있습니다.':!s.approvalRequired?'승인 기반 증강이 설정되지 않았습니다.':p?`${E(p.node)} · ${num(p.qualifiedRps)}건/s · p95 ${num(p.qualifiedP95Milliseconds)}ms`:s.target?'준비 완료 후 신규 요청 경로를 전환합니다.':E(reasons[s.reason]||s.reason||'부하가 지속되면 추천을 표시합니다.')}</p></div><button id="augment-${E(s.uid)}" class="button ${ready?'primary':''}" data-augmentation-approve="${E(s.uid)}" ${!ready||approving.has(s.uid)?'disabled':''} aria-busy="${approving.has(s.uid)}">${approving.has(s.uid)?'<span class="runtime-spinner" aria-hidden="true"></span>승인 접수 중…':ok&&s.target?'<span class="runtime-spinner" aria-hidden="true"></span>모델 준비 중…':'추천 노드 증강 승인'}</button></div>${receipt}${approvalErrors.has(s.uid)?`<p class="runtime-warning" role="alert">${E(approvalErrors.get(s.uid))}</p>`:''}</aside></div><dl class="augmentation-metrics"><div><dt>요청 p95 / 목표</dt><dd>${num(l?.p95Milliseconds)} / ${num(l?.maxP95Milliseconds)} <small>ms</small></dd></div><div><dt>유입 / 완료 처리율</dt><dd>${num(l?.arrivalRps)} / ${num(l?.completedRps)} <small>건/s</small></dd></div><div><dt>대기 / 처리 중</dt><dd>${num(load?.pending)} / ${num(load?.inFlight)} <small>건</small></dd></div><div><dt>검증 수용 요청률</dt><dd>${num(load?.qualifiedRps)} <small>건/s</small></dd></div><div><dt>동시 처리 한도</dt><dd>${num(load?.capacity)} <small>건</small></dd></div><div><dt>실패 / 측정 표본</dt><dd>${num(l?.failures)} / ${num(l?.samples)} <small>건</small></dd></div></dl><footer class="augmentation-footnote"><span>${l?`${num(l.windowSeconds)}초 집계 · ${l.valid?'지연 판단 유효':'표본·오류 확인 필요'}`:'지표 관측 대기'} · ${E(time(s.checkedAt))} · 5초 갱신</span><span>AI 추론만 전환 · 센서 수집 유지 · 저부하 복귀는 기존 정책 적용</span></footer></article>`;
}
...
function tickClock(){if(root.document.hidden)return;const now=Date.now()/1000;root.document.querySelectorAll('[data-runtime-elapsed]').forEach(el=>{el.textContent=Math.max(0,Math.floor(now-Number(el.dataset.runtimeElapsed)))+'초';});}
...
const api={nodeMetrics,nodeMetricsView,augmentationView,serviceMotion,mapNodes,runtimeMap,renderState,targetView,latencyView,activate,setup,render:()=>renderState(entry,Date.now()/1000,expanded)+`<details class="runtime-diagnostics" data-runtime-details="history" ${expanded.has('history')?'open':''}><summary>시험 실행 이력 · 응답 상세</summary>${root.NexusRuntimeDemo?.renderPanel()||''}</details>`};
