...
function renderWorkspace(value,uid,now=observationNow(value)){
 ...
 return picker+operationsView(service,now,current)+(service.managed===false?'':renderState(scoped,now,expanded,true,true)+`<details class="runtime-diagnostics" data-runtime-details="history" ${expanded.has('history')?'open':''}><summary>${E(service.name)} · 부하 실행 이력과 응답</summary>${root.NexusRuntimeDemo?.renderPanel(service.uid)||''}</details>`);
}
function render(){
...
const api={catalogServices,catalogStatus,operationsView,compactRoute,returnView,observationNow,nodeMetrics,nodeMetricsView,augmentationView,serviceMotion,mapNodes,runtimeMap,renderState,selectedService,servicePicker,renderWorkspace,targetView,latencyView,activate,setup,render};
