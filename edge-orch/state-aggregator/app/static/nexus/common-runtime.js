...
function mapNodes(s,m,now){
 ...
 return [...nodes.values()].sort((a,b)=>(a.step||99)-(b.step||99)||(a.role==='server')-(b.role==='server')||a.node.localeCompare(b.node)).map(t=>{
  ...
  const state=!m.ok||(!current&&!preparing&&!retiring&&!recommended&&t.eligible===false)?'unknown':preparing?'preparing':retiring?'retiring':current?(s.serving?'active':'unknown'):recommended?'recommended':'candidate';
