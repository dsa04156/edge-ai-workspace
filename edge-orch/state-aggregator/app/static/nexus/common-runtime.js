...
function serviceMotion(s,now,current){
 ...
 let title=!ok?'최신 관측 확인 불가':s.target?'모델 준비 중':(s.retiring||[]).length?'이전 요청 마무리 중':p?'추천 도착 · 승인 대기':busy?(s.aiInference?'AI 추론 실행 중':'서비스 요청 처리 중'):s.serving?'요청 대기 중':phases[s.phase]||'서비스 준비 확인';
...
function returnView(s,now,current){
 if(!s.augmentationStages?.length)return '';
 ...
 const detail=!r?'최신 복귀 관측을 확인하고 있습니다.':r.phase==='Baseline'?r.label+'에서 서비스를 유지합니다.':r.phase==='Observing'?`요청·대기·처리 중 작업이 없는 ${num(r.windowSeconds)}초 관측과 ${num(r.dwellSeconds)}초 유지 조건을 확인합니다.`:r.phase==='Waiting'?`${r.label}로 복귀 · 최소 ${num(remaining)}초 후 조건 재확인`:r.phase==='Preparing'?`${r.label} 모델 준비가 끝나면 신규 요청 경로를 전환합니다.`:r.phase==='Releasing'?`${r.label}로 경로 전환됨 · 이전 요청 종료와 상위 모델 해제를 확인합니다.`:`${r.label}의 가용성·검증 성능·반환 상태를 확인해야 합니다.`;
 return `<section class="runtime-run-status runtime-return-status" aria-label="자동 축소와 복귀"><div class="runtime-run-title" role="status"><strong>${E(r?headings[r.phase]:'자동 복귀 관측 확인 불가')}</strong></div><p>${E(detail)}</p><small>증강은 직접 승인 · 복귀는 Spark → Orin → Nano 순서로 자동 진행 · 새 요청이 오면 무부하 대기 재확인</small></section>`;
}
...
function renderState(entry,now=observationNow(entry),expanded=new Set(),selectedView=false){
...
function servicePicker(value,uid,now){
...
function renderWorkspace(value,uid,now=observationNow(value)){
 ...
 const service=selectedService(value.data.services,uid);
 const picker=servicePicker(value,service?.uid||uid,now);
 ...
 return picker+renderState(scoped,now,expanded,true)+`<details class="runtime-diagnostics" data-runtime-details="history" ${expanded.has('history')?'open':''}><summary>${E(service.name)} · 부하 실행 이력과 응답</summary>${root.NexusRuntimeDemo?.renderPanel(service.uid)||''}</details>`;
}
function render(){
