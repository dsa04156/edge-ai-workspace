...
function details(p,a,d){
 ...
 const svc=a?.services?.find(x=>x.service_id===contextService)||a?.service,valid=d.valid.profiles,perf=(svc?.service_id==='sensor-anomaly-demo'||example)&&d.valid.demo?d.demo?.performance:null;
...
function historyPanel(){return `<details class="sm-history" data-sm-disclosure="history" ${disclosures.has('history')?'open':''}><summary>배치 변화 이력 <span>${events.length}건 · 이 화면을 연 이후</span></summary>${events.length?`<ol>${events.map(e=>`<li><time>${E(clock(e.time))}</time><div><strong>${E(e.text)}</strong><span>${E(e.id)}</span><small>${E((e.from||[]).join(', ')||'미관측')} → ${E((e.to||[]).join(', ')||'미관측')}</small></div></li>`).join('')}</ol>`:'<p class="sm-caption">아직 새로운 배치 변화가 관측되지 않았습니다. Pod 배치의 변화는 서비스 실행 권한 전환이나 무중단 이동을 증명하지 않습니다.</p>'}</details>`;}
function html(){
 ...
 return ` ${example?`<div class="sm-example-banner"><div><b>예시 ${step+1}/4 · ${scenarios[step].title}</b><p>${scenarios[step].note}</p></div><div><button class="button" id="sm-play" data-sm-action="play">${playing?'예시 일시정지':'예시 자동 재생'}</button><button class="button" id="sm-next" data-sm-action="next">다음 장면 →</button></div></div>`:''}
 ...
 ${example?'':historyPanel()}</div><aside class="sm-inspector" aria-label="선택한 실행체 상세">${details(chosen,a,d)}</aside></div>
