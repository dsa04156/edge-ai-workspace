...
function chooseService(id){
 serviceSelection=id;selected=null;contextService=id;showEmpty=id==='*';scope='linked';
 const url=new URL(root.location.href);url.searchParams.delete('service');url.searchParams.delete('workloads');if(id==='*')url.searchParams.set('workloads','all');else if(id)url.searchParams.set('service',id);url.hash='service-map';root.history.pushState(null,'',url);root.NexusLive?.setServiceId?.(id==='*'?null:id);paint();root.NexusLive?.refreshView?.();
}
function html(){
...
function paint(){const el=root.document?.getElementById('service-placement-map');if(!el)return;const focused=root.document.activeElement,id=focused?.id,catalogId=focused?.dataset?.smService,sel=focused?.dataset?.smSelect,node=focused?.dataset?.smNode;
 ...
 el.classList.toggle('is-example',example);el.classList.toggle('is-equipment',!example&&serviceSelection==='*');el.innerHTML=html();
...
const api={render,sourceGroups,isOverview:()=>new URLSearchParams(root.location.search).get('workloads')==='all',openOverview:()=>chooseService('*'),openCatalog:()=>chooseService(null),serviceDevices,serviceKeys,scopedProfiles,validate,associations,placements,changes,execution,routeState,advance,fresh,recent,escape:E};
