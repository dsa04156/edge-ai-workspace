...
function renderNodeConfig(node) {
  ...
  const target = targetById(node.targetId);
  const propertyOptions = (target?.properties || []).map((property) => `<option value="${workflowEscape(property)}" ${node.config.property === property ? "selected" : ""}>${workflowEscape(property)}</option>`).join("");
  const modeOptions = sourceReadModesForTarget(target).map((mode) => `<option value="${workflowEscape(mode)}" ${node.config.readMode === mode ? "selected" : ""}>${workflowEscape(DEVICE_SOURCE_MODE_LABELS[mode])}</option>`).join("");
  const sourceSettings = node.type === "device_source"
    ? `<div><dt>읽기 방식</dt><dd><select class="workflow-config-input" data-config-field="readMode">${modeOptions}</select></dd></div>${node.config.readMode !== "local_latest" ? `<div><dt>조회 구간</dt><dd><input class="workflow-config-input" data-config-field="window" value="${workflowEscape(node.config.window || "-10s")}" /></dd></div>` : ""}<div><dt>리소스</dt><dd><select class="workflow-config-input" data-config-field="property"><option value="auto">자동 선택</option>${propertyOptions}</select></dd></div>`
    : "";
  return `<div class="inspector-title"><span>단계 설정</span><strong>${workflowEscape(node.label)}</strong></div><dl class="workflow-fields"><div><dt>유형</dt><dd>${workflowEscape(nodeTemplate(node.type).label)}</dd></div><div><dt>이름</dt><dd><input class="workflow-config-input" data-config-field="label" value="${workflowEscape(node.label)}" /></dd></div>${targetOptions ? `<div><dt>대상</dt><dd><select class="workflow-config-input" data-config-field="targetId"><option value="">선택 안 함</option>${targetOptions}</select></dd></div>` : ""}${sourceSettings}${node.type === "condition" ? `<div><dt>rule</dt><dd><input class="workflow-config-input" data-config-field="value" value="${workflowEscape(node.config.value || "true")}" /></dd></div>` : ""}${node.type === "ai_inference" ? `<div><dt>model</dt><dd><input class="workflow-config-input" data-config-field="model" value="${workflowEscape(node.config.model || "anomaly-lite")}" /></dd></div>` : ""}${node.type === "postprocess" ? `<div><dt>threshold</dt><dd><input class="workflow-config-input" data-config-field="threshold" value="${workflowEscape(node.config.threshold || "0.82")}" /></dd></div>` : ""}${node.type === "store_observe" ? `<div><dt>sink</dt><dd><input class="workflow-config-input" data-config-field="sink" value="${workflowEscape(node.config.sink || "EdgeX Core Data / PostgreSQL")}" /></dd></div>` : ""}</dl>`;
}
...
function renderTargetInspector(node, target, latestPayload) {
  ...
  const samples = Array.isArray(latestPayload) ? latestPayload : Array.isArray(latestPayload?.samples) ? latestPayload.samples : [];
  const latestRows = samples.slice(-6).reverse().map((item) => `<li>${workflowEscape(item.resourceName || item.resource_name || item.property)} = ${workflowEscape(item.value)} · ${workflowEscape(item.timestamp)}</li>`).join("");
  const sourceKind = latestPayload?.sourceKind === "device_service_local_cache" ? "Device Service 로컬 캐시 · 휘발성" : latestPayload?.sourceKind === "edgex_core_data" ? "중앙 Core Data · 영구 저장" : "";
  const retention = latestPayload?.retention ? `${workflowText(latestPayload.retention.maxAge)} / 최대 ${workflowText(latestPayload.retention.maxSamples)}개` : "";
  return `<div class="workflow-source-list"><span>바인딩 대상</span><dl class="workflow-fields compact"><div><dt>대상</dt><dd>${workflowEscape(target.displayName)}</dd></div><div><dt>노드</dt><dd>${workflowEscape(target.nodeName || "미지정")}</dd></div><div><dt>조회 방식</dt><dd>${workflowEscape(DEVICE_SOURCE_MODE_LABELS[node.config.readMode] || node.config.readMode || "history")}</dd></div><div><dt>텔레메트리</dt><dd>${workflowEscape(target.telemetryStatus)} · ${workflowEscape(target.telemetryLastSeenAt || "샘플 없음")}</dd></div><div><dt>상태</dt><dd>${workflowEscape(target.overallStatus)} · ${workflowEscape(target.reason)}</dd></div></dl><ul>${target.properties.length ? target.properties.slice(0, 8).map((item) => `<li>${workflowEscape(item)}</li>`).join("") : "<li>읽을 수 있는 리소스 없음</li>"}</ul></div>${latestRows ? `<div class="workflow-source-list"><span>샘플 미리보기</span><p class="workflow-message-text">${workflowEscape(sourceKind)}${retention ? ` · ${workflowEscape(retention)}` : ""}</p><ul>${latestRows}</ul></div>` : ""}`;
}
