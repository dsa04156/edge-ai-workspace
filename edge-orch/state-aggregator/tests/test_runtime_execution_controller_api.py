...
class FakeController:
    ...
    async def execute(self, plan, approval):
        ...
        return RuntimeExecutionRecord(
            ...
            plan=plan,
            steps=[RuntimeExecutionStepState(step_id=s.step_id, action=s.action, status=("SUCCEEDED" if i < 2 else "BLOCKED")) for i, s in enumerate(plan.steps)],
            updated_at=NOW,
        )
...
def test_dry_run_and_approved_execution_api(monkeypatch) -> None:
    ...
    assert controller.dry_run_calls == 1 and controller.execute_calls == 1
