...
## 최신 추가: 실제 DGX Spark GPU 실측 (2026-09-09 측정, 09-10 반영)
...
이 시험은 아래 네 장비의 **입력 512 / 출력 128토큰 공통 설정 시험과 다르다**.
...
장비별 공통 설정 성능비를 대체하지 않는다. Spark의 512/128토큰 공통 설정 sweep은 아직 미측정이다.

## 공통 설정 비교: 다양한 요청의 GPU 시험 (2026-09-08 후속)

**Nano·실제 AGX Orin·RTX 5060 Ti·RTX 5080 모두 장비 내부에서 측정했다.**
이번에는 승인받아 Nano 기존 시연 모델을 잠시 내리고 공통 설정 모델만 실행했으며,
5080 센서 추론의 GPU도 한시적으로 빌렸다. Spark는 이 공통 설정 비교에는 미참여이며,
위 2026-09-09의 8토큰 데모 조건 실측을 별도로 제공한다.
...
| 항목 | 이번 실험 조건 |
|---|---|
| 모델 | **Llama 3.2-1B Instruct · Q8_0 GGUF**, 네 장비에 동일 파일 사용 |
...
| 실행 위치 | Nano·AGX·5060 Ti: 장비의 독립 프로세스. 5080: GPU를 할당받은 Kubernetes Pod. **요청 발생기도 각 장비 내부** |
...
| **RTX 5080** | **0.30초** | **2.28초** | 측정한 네 장비 중 가장 짧음 |
| **DGX Spark** | 이 조건 미측정 | 이 조건 미측정 | 위 8토큰 조건의 실측 참고. 직접 성능비 비교 불가 |
...
최종 탐색은 **202건 예정 → 201건 전송·성공, 1건 미전송**이었다. 미전송은 5080의
...
`sweep-rtx5060ti-a/sweep`, `sweep-rtx5080-c/sweep`.
...
Nano·AGX·5060 Ti는 호스트 프로세스와 로컬 요청 발생기, 5080은 GPU Pod 내 서버와 같은
...
C8의 표본 p95 TTFT는 각각 **22.814·14.223·3.577·2.010초**였다.
동시 요청을 늘려도 완료 처리량은 기본 512/128 조건에서 약
**0.295·0.475·1.88·3.37 요청/s**로 비슷했다. 이는 고정 slot 1의 관측값이지
...
| 입력/출력 토큰 | Nano | AGX Orin | RTX 5060 Ti | RTX 5080 |
|---|---:|---:|---:|---:|
| 128/128 | 3.242 | 1.960 | 0.513 | 0.285 |
| 512/128 | 3.384 | 2.101 | 0.537 | 0.298 |
| 2048/128 | 4.971 | 3.069 | 0.741 | 0.416 |
| 512/64 | 1.832 | 1.137 | 0.287 | 0.170 |
| 512/256 | 6.536 | 4.060 | 1.031 | 0.577 |
| 2048/256 | 8.321 | 5.120 | 1.262 | 0.712 |
...
AGX 33/33, 5060 Ti 57/57, 5080 예정 82건 중 전송 81/완료 81/미전송 1이다.
...
Nano 약 0.30, AGX 약 0.48, 5060 Ti 약 1.9, 5080 약 3.4 요청/s다.
...
`sweep-rtx5060ti-a/sweep`, `sweep-rtx5080-c/sweep`에 있다.
...
| NVIDIA DGX Spark | 접속 가능한 실제 장비 미확인 | 미검증 | RTX 5080으로 대체 표기하지 않음 |
