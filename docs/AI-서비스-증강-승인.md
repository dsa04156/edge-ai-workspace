...
> 2026-09-11 후속 수정: 아래 노드별 부하의 수동 종료 동작은 코드·테스트 완료,
> 운영 배포 대기다. 이미지 저장소 `192.168.0.56:5000`이 연결을 거부해 이미지는 바꾸지 않았다.
> 운영의 기존 노드 부하는 아직 40초·512건 제한이 적용된다.
...
### 노드 부하 자동 종료 수정 (2026-09-11, 배포 대기)
...
- 저장소 연결 거부로 빌드·운영 배포와 40초 초과 실장비 검증은 미완료다.
- 근거: `edge-orch/runtime-operator/results/2026-09-11-manual-node-load/diagnosis.json`.
