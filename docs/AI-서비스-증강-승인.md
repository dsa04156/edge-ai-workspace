...
따르고 NEXUS에서 임의 노드·image·command를 받지 않는다. 부하 시험은 등록된 입력과 동시성 제한을 사용한다. 서비스 부하는 노드 전환 후에도 제거까지 유지한다. 개별 노드 부하는 제거 또는 실행 노드 변경까지
...
1. NEXUS `OFFLOAD DEMO`에서 `llama-inference`를 선택한다. 현재 실행 노드·지표·이력이 함께 바뀐다.
2. 중지된 서비스라면 `서비스 실행`을 누른 뒤 `서비스 부하 시작`을 누른다.
   고정 입력을 동시 6건으로 반복한다. 승인 후 Orin·Spark로 이동해도 부하는 유지한다.
...
4. `승인 접수` → `모델 준비 중` → `증강 적용 · 신규 요청 경로 전환 완료`를 확인한다.
   ...
   서비스 부하 영역의 `부하 제거`는 새 요청 생성을 멈추고 이미 처리 중인 요청을 마무리한다.
...
- Nano·Orin·Spark 카드의 개별 부하 시험을 펼치면 부하 주기/부하 제거 버튼을 표시한다. 시작은 최신 관측에서
...
운영 검증: Argo Synced/Healthy와 Ready image·JS2개·HTML의 운영 파일 및 HTTP 해시 일치를
확인했다. 새 브라우저에서 시계 -0.5초/-60초/+60초와 정상시각 모두 요청 대기 상태를 유지했다.
관측 API 오류 빈 응답, 네트워크 실패, 4.5초 응답 시간 초과를 브라우저 안에서만 주입하여
지도·마지막 상태 보존과 오류 표시, 정상 응답 후 복구를 확인했다. 390px 가로 넘침/중첩 버튼0.
제어기 이미지는 변경하지 않았으며 부하·실행·승인 동작을 시험으로 실행하지 않았다.
현재 Orin Serving 관측 정상. `verification.json`에 근거와 사용자 PC 미측정 한계를 기록했다.


## 2026-09-14 무부하 자동 축소·복귀

부하가 완전히 없어지면 추가 요청을 만들어 지연 표본을 채우지 않고 별도 무부하 조건으로 복귀한다. Spark → Orin → Nano 순서이며 바로 이전 단계가 준비되지 않으면 건너뛰지 않는다. 현재 계약에서는 각 단계의 20초 무요청 관측과 8초 유지 뒤 모델 준비를 시작하며, polling·모델 준비·이전 모델 해제 시간 때문에 실제 전환 완료는 더 늦을 수 있다.

- gateway 유입·응답 기록이 최근 window에 없고 대기·처리 및 worker 처리도 0건이어야 한다. 실패 기록은 만료 전까지 무부하로 취급하지 않는다. p95 표본 부족은 계속 표본 부족으로 표시한다.
