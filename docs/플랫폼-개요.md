...
| 운영 화면 | `state-aggregator` | 권위 데이터 결합, freshness·상태 계산, read-only API |
...
기본 실행은 Device1의 고정 edge-local 경로다. Server1 endpoint는 별도 overlay로 검증됐지만
승인 기반 선택 경로이며 자동 증설이나 범용 offloading이 아니다. 설비 이상 점수와 컴퓨팅
자원 증강 판단은 서로 독립이다. 입력·판단·출력과 `BLOCKED` 의미는
