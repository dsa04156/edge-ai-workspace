...
### 노드 상태 가시성 보완 (2026-09-08)

운영 현황 첫 영역과 자원 상세에 노드별 상태를 직접 표시한다. `/api/resources`의
`kubernetesReady`는 Kubernetes Ready condition으로 계산하며 기존 자원 health와
별도 필드다. `/state/nodes`의 CPU·메모리·네트워크 압박과 metric을 함께 표시하되
60초가 지난 관측 또는 조회 실패는 현재 정상으로 표시하지 않는다. GPU 미관측과
사용률 0%를 구분하며 수집기 응답을 센서 연결 상태로 해석하지 않는다.
