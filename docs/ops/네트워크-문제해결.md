...
이 조치는 애플리케이션에 ClusterIP나 PodIP를 고정하지 않는다. cloud-only Service만 기존 kube-proxy가 처리하고, edge workload는 계속 DNS 이름을 사용한다. `edgex-ingest-gateway`는 필터링하지 않으므로 Jetson/Sense HAT에서 gateway로 가는 edge-to-cloud 경로는 EdgeMesh를 유지한다.
