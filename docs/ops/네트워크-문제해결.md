...
## 6. cloud-only Service의 hybrid proxy 필터

2026-07-21 점검에서 `etri-ser0002-cgnmsb` Pod의 CoreDNS endpoint IP 직접 질의는 성공했지만 `kube-dns` Service DNS 질의는 timeout이 났다. EdgeMesh 로그에서는 server2에서 control-plane으로 가는 UDP/TCP proxy stream reset이 확인됐다. EdgeMesh와 kube-proxy가 같이 있는 cloud 노드에서 edge 접근이 필요 없는 Service는 EdgeMesh의 공식 hybrid-proxy 필터 라벨로 제외한다.

현재 운영 필터는 다음과 같다.

```bash
kubectl -n kube-system label service kube-dns \
  'service.edgemesh.kubeedge.io/service-proxy-name=' --overwrite
kubectl -n argocd label service argocd-repo-server argocd-redis \
  'service.edgemesh.kubeedge.io/service-proxy-name=' --overwrite
```

이 조치는 애플리케이션에 ClusterIP나 PodIP를 고정하지 않는다. cloud-only Service만 기존 kube-proxy가 처리하고, edge workload는 계속 DNS 이름을 사용한다. `edgex-ingest-gateway`는 중앙 호환 구성을 위해 필터링하지 않지만 현재 Jetson Serial과 Raspberry Pi I2C Device Service는 gateway를 호출하지 않고 Keeper/Metadata/MessageBus/Command Service FQDN을 사용한다.

확인 항목:

- server2 Pod에서 `kube-dns`, `argocd-repo-server`, `argocd-redis`가 DNS 이름으로 해석된다.
- 필터된 Service는 `KUBE-PORTALS-*`에서 사라지고 `KUBE-SERVICES`에는 남는다.
- Argo CD Application이 다시 `Synced` 상태로 갱신된다.
- EdgeMesh DaemonSet은 설정 변경이나 Pod 재시작 없이 기존 UID를 유지한다.

Service가 재생성되거나 Argo CD/cluster addon을 재설치한 뒤에는 라벨을 재확인한다.
