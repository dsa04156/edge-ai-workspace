...
4. Device Service·Core Data·서비스 입력 오류 진단
...
현재 checkout에는 펌프·모터 `online-baseline` 서비스와 결과·알림 저장이 있다. 이를 옥동
실모델로 표시하지 않으며 생산품질 판별은 계약 단계다. workflow builder는
validation/plan preview만 제공한다.
...
- 운영 KPI: 수집 성공률, 장애 복구 시간, 후속 장애 저장·재전송 경로 구축 시 replay 성공률

AI 모델 KPI와 물리 장비 availability KPI의 분모를 섞지 않는다. 세부 정의는 [옥동 생산성 KPI](../옥동-생산성-kpi.md)를 따른다.

## 저장소 표현

Core Data/PostgreSQL은 장기 Event/Reading 권위이며 AI 파생 결과는 서비스 저장소가 소유한다.
Device Service Local Data와 애플리케이션 cache는 최근 입력용이며 durable outbox가 아니다.
선택적 InfluxDB resource-profile 기록은 물리 telemetry 권위가 아니다.
