...
`운영 현황`은 이 네 분류의 수와 가용 상태 다음에 중앙 서버와 물리 디바이스 상태를
분리해 보여 준다. `/state/dashboard`의 node snapshot을 사용해 각 분류의 평균
CPU·메모리, GPU 관측 대수, pressure 주의 대수와 노드별 현재 meter를 표시한다.
이 값은 Prometheus/Kubernetes의 현재 관측이며 과거 시계열이나 Grafana embed가 아니다.
