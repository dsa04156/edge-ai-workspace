...
> 상태: 설계 이력. 이 문서의 `factory-vision-inspection-ai`와 고정 후보/결과 값은 실제 센서 입력이 없는 과거 시나리오 계약이며 현재 운영 서비스 데모가 아니다. 해당 Argo CD Application은 2026-07-22 운영 경로에서 퇴역 대상으로 전환했다. 현재 서비스 데모는 [실측 센서 이상 탐지 시나리오](서비스-데모-시나리오.md)를 따른다.
