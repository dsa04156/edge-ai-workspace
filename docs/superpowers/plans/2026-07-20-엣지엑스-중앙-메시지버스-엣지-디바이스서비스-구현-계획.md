# 엣지엑스 중앙 메시지버스와 엣지 디바이스 서비스 구현 계획

> **상태: 폐기된 구현 계획 / 실행 금지.** 현재 root Kustomize는 HTTPS ingest plane을 배포한다. 이 문서의 `telemetry` namespace 삭제나 Device MQTT 재배치 명령을 실행하지 않는다. 최신 기준은 `../../원시-텔레메트리-데이터-플레인.md`와 `../specs/2026-07-14-엣지엑스-중앙-코어-엣지-버퍼-설계.md`다.
...
```bash
rtk git add docs/프로젝트-배경.md docs/프로젝트-범위.md docs/저장소-구조.md docs/원시-텔레메트리-데이터-플레인.md docs/ops/runbook-central-messagebus-rebuild.md
