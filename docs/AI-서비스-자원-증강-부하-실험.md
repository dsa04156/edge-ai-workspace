...
>
> **Jetson 비교 결론:** Jetson Device1도 같은 연산급에서 Server1이 12/12회 지연 우위를
...
**표 1. 장비별 오프로딩 검토 구간과 실험 결과**

| 장비 | 오프로딩 검토 구간 | 실험 결과 |
|---|---|---|
| Jetson | 약 1,180만 연산 + CPU 95%↑ + p95 95ms↑ 또는 처리량 12.3/s↓ | 9/12 통과 → 조건부 권고 |
| Raspberry Pi 5 | 약 1,180만 연산 + CPU 95%↑ + p95 90ms↑ 또는 처리량 20/s↓ | 12/12 통과 → 권고 가능 |
| 경량 모델 | 계산량이 작음 | 네트워크 비용이 더 커서 로컬 유지 |
...
노드 전체 사용률과 AI 서비스 Pod의 CPU 포화도는 다른 지표다. 노드 CPU·메모리 85%는
다른 workload까지 포함한 **조사 시작 기준**이고, 실험 Pod CPU 포화도 95%는 할당된 CPU
quota를 AI 계산이 사실상 모두 사용하는 **장치별 병목 기준**이다.

**표 4. 노드 및 AI Pod 부하별 오프로딩 판단**

| 관측 상태 | 운영 판단 |
|---|---|
| 노드 CPU·메모리 85% 미만이고 서비스 지연·처리량 저하가 없음 | 로컬 처리를 유지한다. |
| 노드 CPU 또는 메모리 85% 이상이 300초 지속 | 원인을 조사한다. 이 조건만으로 오프로딩하지 않는다. |
| AI 서비스 Pod CPU 포화도 95% 미만이거나 장치별 p95·처리량 경계를 넘지 않음 | 로컬 처리를 유지하고 관찰한다. |
| Pod CPU 포화도 95% 이상 **그리고** 장치별 p95·처리량 저하가 180초 지속 | 동일 모델의 Server1 후보 시험 결과를 확인한다. |
| 위 조건에서 Server1이 지연 10% 개선·처리량 감소 5% 이내·오류 0을 만족 | 오프로딩을 `RECOMMENDED`로 권고한다. |

실험에서는 CPU 경쟁 부하를 0%·50%·75%·100%로 바꿔 비교했다. Raspberry Pi는 약
1,180만 연산 모델의 네 단계 모두에서 Server1 gate를 통과했다. Jetson은 0%·50%·100%
...
3. **약 295만 연산 대표 모델:** 일부 조건에서 Server1이 빨랐지만 부하 전 범위에서 결과가
   ...
   채택하지 않았다.
4. **Jetson·약 1,180만 연산 대표 모델:** 동일 입력 12회 비교에서 Server1이 모두 지연
   우위를 보였지만 최종 후보 gate 통과는 9/12회였다. 중앙값 기준 지연은 50.8% 감소하고
   처리량은 2.452배로 증가했다. CPU 경쟁 부하 75% 조건은 지연 개선이 9.9%여서 사전에
   정한 10% gate에 0.1%p 부족했으며, 이를 반올림해 통과로 바꾸지 않았다.
5. **Raspberry Pi·약 1,180만 연산 대표 모델:** 로컬 p95가 약 90~196ms, 처리량이 약
   6~20건/s로 저하됐다. 동일 입력 12회 비교에서 Server1이 모두 지연 우위를 보였고 12/12
   run이 후보 gate를 통과했다. 중앙값 기준 지연은 48.7% 감소하고 처리량은 3.806배로
   증가했다. Jetson과 Raspberry Pi의 지연 방향 양측 sign test는 각각 `p=0.000488`이었다.
6. **전체 경로 측정:** Server1 지연에는 요청 직렬화, 네트워크 전송, 서버 처리와 결과 반환을
...
> Jetson Device1에서는 약 1,180만 연산급 모델을 실행할 때 CPU가 95% 이상 포화되고,
> 로컬 p95가 95ms 이상이거나 처리량이 초당 12.3건 이하인 상태가 180초 이상 지속되면
> Server1 오프로딩을 조건부로 검토한다. Server1은 12회 모두 지연 우위를 보였지만 최종
> gate는 9회만 통과했으므로, 현재 부하 조건에서 10% 지연 개선·5% 처리량 비열등 기준을
> 다시 통과한 경우에만 `RECOMMENDED`로 표시한다.
>
...
**표 9. 실제 Arduino 데이터 엣지↔Server1 비교 결과**
...
**표 10. 대표 AI 모델 연산량·CPU 부하별 오프로딩 교차점**
...
**표 11. Jetson·Raspberry Pi 동일 입력 비교 결과**
...
**표 12. 엣지·모델 프로필별 Server1 후보 자격 결과**
