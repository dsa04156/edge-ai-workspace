...
이번 실험의 핵심은 **모든 AI 작업을 Server1으로 보내는 것이 아니라, 엣지에서 처리하기
버거운 무거운 모델만 오프로딩 후보로 삼는 것**이다.

| 비교 대상 | 실제로 관측한 결과 | 운영 해석 |
|---|---|---|
| 현재 가벼운 기준선 모델 | Device1 로컬이 Server1보다 빨랐다. 실제 입력에 가까운 1 RPS에서 로컬은 1.265ms, Server1은 15.633ms였다. | 네트워크·HTTP 왕복 시간이 계산 이득보다 크므로 **로컬 처리를 유지**한다. |
| 약 295만 연산 대표 모델 | 일부 조건에서는 Server1이 빨랐지만 CPU 부하가 달라지면 결과가 일정하지 않았다. | 안정적인 전환 기준으로 쓰지 않고 **오프로딩 대상에서 제외**한다. |
| 약 1,180만 연산 대표 모델 | Raspberry Pi CPU가 포화되면서 로컬 p95가 약 90~196ms, 처리량이 약 6~20건/s까지 내려갔다. Server1은 12회 비교 모두에서 더 유리했다. | 아래 조건이 지속되면 **Server1 오프로딩 검토를 권고**한다. |

쉽게 말하면 Raspberry Pi가 1,180만 연산급 모델을 혼자 처리할 때 CPU가 꽉 차고, 요청
100개 중 느린 쪽 5개가 90ms 이상 걸리거나 초당 완료 건수가 20건 이하로 떨어지는 구간에서
서버로 보내는 편이 이득이었다. 비교 run의 중앙값 기준으로 Server1은 지연을 약 48.7%
줄였고 처리량을 약 3.8배 높였다.

여기서 `RPS`는 1초에 들어오는 요청 수, `처리량`은 1초에 실제로 끝낸 요청 수다. 예를 들어
50 RPS가 들어오는데 처리량이 20건/s라면 초당 약 30건이 대기열에 쌓이는 병목 상태다.
`p95 90ms`는 요청 100개 가운데 약 95개는 90ms 안에 끝나지만 느린 쪽 약 5개는 그보다
오래 걸린다는 뜻이다.

운영에서는 이 수치만 한 번 넘었다고 바로 전환하지 않는다. 같은 상태가 180초 동안
지속되고, 같은 모델을 실행한 Server1이 지연을 10% 이상 줄이면서 처리량은 5% 넘게
떨어뜨리지 않아야 `RECOMMENDED`로 표시한다. 현재 화면은 권고만 제공하며 자동으로
워크로드를 이동하거나 요청 경로를 바꾸지 않는다. 또한 약 1,180만 연산 모델은 실제 센서
값으로 계산 부하를 비교하기 위한 성능 프록시이므로, 실제 옥동 학습 모델이 준비되면 같은
절차로 다시 검증해야 한다.
