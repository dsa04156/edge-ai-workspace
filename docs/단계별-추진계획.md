...
physical I2C / Serial / Modbus / OPC-UA device
  ...
  -> HTTPS/mTLS -> central ingest gateway -> Core Data
...
현재 repository delivered 단계는 loopback Protocol Adapter 계약, direct-mode agent,
SQLite outbox, HTTPS ingest gateway, Metadata bootstrap과 EdgeX 기반
state-aggregator/dashboard 계약이다. 새 image의 실제 운영 환경 cutover는 별도 smoke로
확인해야 한다. `sensehat-001`은 교체 가능한 검증 fixture이며 Serial, Modbus, OPC-UA와 RTSP workflow는 protocol별 migration gate를
통과하기 전까지 완료로 주장하지 않는다. 과거 Jetson MQTT mode는 현재 render에서 제거된 비교 기록이다.
