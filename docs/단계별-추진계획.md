...
2026-07-21 현재 중앙 EdgeX와 EdgeX 기반 state-aggregator/dashboard 계약은 유지되지만,
기존 direct Agent/SQLite outbox/Metadata bootstrap은 퇴역했다. `edgex-edge`에는 아직
Device Service가 없으므로 telemetry는 전환 중이다. 다음 단계는 Jetson Arduino Serial
Device Service/Profile/Device와 실제 Event/Reading을 종단간 검증하는 것이다. Modbus,
OPC-UA, 추가 I2C와 RTSP workflow는 protocol별 migration gate 전까지 완료로 주장하지 않는다.
