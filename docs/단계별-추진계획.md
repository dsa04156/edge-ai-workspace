...
2026-07-21 현재 기존 direct Agent/SQLite outbox/Metadata bootstrap은 퇴역했다.
`edgex-edge`의 Jetson Arduino Serial Device Service/Profile/Device는 실제 Event/Reading,
read-only command와 state-aggregator/dashboard freshness까지 정상 경로를 종단간 검증했다.
다음 검증은 실제 장치 분리·복구이며, Modbus, OPC-UA, 추가 I2C와 RTSP workflow는
