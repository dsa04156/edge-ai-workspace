...
func (api *localDataAPI) latest(context echo.Context) error {
	...
	if !known {
		return api.writeError(
			context,
			http.StatusNotFound,
			fmt.Sprintf("unknown local data source %s/%s", deviceName, resourceName),
		)
	}
	...
	if !found {
		return api.writeError(
			context,
			http.StatusNotFound,
			fmt.Sprintf("latest sample is unavailable for %s/%s", deviceName, resourceName),
		)
	}
	return context.JSON(http.StatusOK, api.response(
		deviceName,
		resourceName,
		[]cachedSample{sample},
	))
...
func (api *localDataAPI) response(
	deviceName string,
	resourceName string,
