...
func (cache *recentCache) latest(deviceName string, resourceName string, now int64) (cachedSample, bool) {
	samples := cache.query(deviceName, resourceName, now-cache.maxAge.Nanoseconds(), now, 1, now)
	if len(samples) == 0 {
...
func (cache *recentCache) query(
	...
	start := sort.Search(len(samples), func(index int) bool { return samples[index].Origin >= from })
	end := sort.Search(len(samples), func(index int) bool { return samples[index].Origin > to })
	if start >= end {
		return []cachedSample{}
	}
	if end-start > limit {
