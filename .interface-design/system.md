...
### AI 서비스 수명주기와 노드별 부하 제어

서비스 실행/중지 44px 버튼을 우측 제어부 첫 행에 놓는다. 노드 카드의 상세 선택 버튼과
부하 제어는 별도 요소이며 중첩 버튼을 사용하지 않는다. 각 카드 아래 개별 부하 시험을 펼치면 부하 주기/부하 제거를
표시하고 desktop 세로/mobile 2열, 최소44px·gap6px·padding8px·radius8px로 기존 토큰을 쓴다.
현재 경로만 부하 시작이 가능하며 비활성 이유, 노드별 전송/성공/취소/실패, 제거 진행을 표시한다.
서비스 중지는 요청 drain과 모델 해제, 부하 제거는 서비스 유지라는 차이를 짧은 문장으로 명시한다.


### 선택 서비스 운영 공간 (2026-09-14)

- 상단 서비스 선택은 desktop 3열, 600px 이하 1열이다. 12px gap·8px radius,
  선택 항목은 기존 accent 선·얕은 배경·aria-pressed로 표시한다.
- 서비스명, 실제 AI/합성 HTTP 구분, 현재 상태와 실행 노드를 함께 표시한다.
  UID별 지도·지표·동작·이력을 아래 한 공간에 표시하며 선택 유지에는 runtimeService를 쓴다.
- 지도 아래 실행/중지, 서비스 부하, 배치/승인은 3열·20px gap이며 850px 이하 단일열이다.
  부하 시작·제거는 항상 나란히 표시하고 노드별 시험은 native details 안에 보존한다.
- 서비스 부하가 다른 노드로 이어지는 상태와 개별 노드 시험 종료를 구별한다.
  오래된 관측에서는 실행 중을 단정하지 않으며 마지막 run ID 제거 동작은 유지한다.


### AI 서비스 목록 운영 (2026-09-14 후속)

이 패턴이 이전 3열 선택 카드를 대체한다. 서비스명·상태·실행 위치·운영 방식·부하를
행 목록으로 표시한다. 이름 버튼은 최소44px, 선택 행은 accent inset 선과 얕은 배경이다.
선택 상세는 지표 → compact 실행 경로 → 실행/부하/정책 3열 순서,16px padding과20px gap이다.
노드 기본 표시는 CPU·메모리·GPU·GPU 온도이며 노드 클릭 시 전체 온도와 개별 부하를 펼친다.
600px 이하에서 목록만 내부 가로 스크롤, 실행 경로와 제어는 세로 배열한다.

노드별 부하 제어는 접힘/노드 상세 안에 숨기지 않는다. 각 실행 경로 노드 바로 아래에
부하 주기·제거 44px 버튼을 2열로 항상 표시하고 실행·제거·이동 종료 상태를 함께 둔다.


### 노드 부하 단일 진입과 복귀 근거 (2026-09-14 후속)

AI 공통 서비스 부하 버튼을 제거하고 기존 노드별 44px 시작/제거 쌍을 단일 진입점으로 사용한다.
서비스 이동 후 현재 노드에 동일 run ID의 제거 버튼과 진행 상태를 표시한다. 시작 노드는
부하가 이동한 위치를 안내한다. 기존 3열 하단 가운데는 부하 진행 상태, 오른쪽은 정책 근거다.
대기·지연 지속 시간, 복귀 유지 시간, 하위 검증 요청률 대비 허용률과 cooldown을 12px 보조 정보로
표시한다. 관측 중/복귀 대기/용량 부족 보류는 문구로 구분하며 오래된 관측의 진행값은 숨긴다.


### 요청 진단 기록 (2026-09-14)

선택 서비스 아래에 기존 NEXUS 표·44px 버튼을 재사용한다. 실패·미확인/전체 요청 버튼과
수동 새로고침, 이전 기록 추가 조회를 제공한다. 요청 ID 상세에서 진입 노드·실행 revision·시험
ID·전체 시간을 펼친다. 현재 처리 노드와 미전송, gateway 대기·worker 왕복·보고된 추론 시간을
구분하며 미수집 시간은 0으로 표시하지 않는다. 행 상세 펼침은 5초 화면 재그리기에도 유지한다.
모바일은 페이지 넘침 없이 표 내부에서 가로 스크롤하며 조회 중·실패·빈 기록 상태를 표시한다.
...
### 전체 노드와 서비스별 배치 근거 (2026-09-14)

기존 서비스 운영·부하 제어 아래에 읽기 전용 전체 후보 패널을 둔다. 요약4열, 판단 단계,
등록 실행 설명서, 전체/등록·연결/현재제외 필터, 노드 표와 선택 상세 순서다.
현재 처리 노드는 accent 선으로 표시하고 가용 후보와 제외는 실행 형태별 판단을 합쳐
표시한다. 정지·관측 불가에서는 과거 제외를 현재 판단으로 표시하지 않는다.
CPU·메모리 예약 차감 잔량과 실측 사용률을 서로 다른 열에 둔다.
44px 필터·노드 버튼, 표 최소1020px 내부 가로 스크롤,900px 이하 요약2열,
600px 이하 실행 설명서·상세1열을 사용한다. 입력·성능·자원 정보 미연결은 명시한다.


### 서비스 실행·후보 순위 맵 (2026-09-14)

선택 서비스 pill에서 노드 카드로 분기선을 연결한다. 기본 3열, 추가 후보는 다음 행,
650px 이하 단일열이다. 현재 요청 경로는 accent 실선, 준비 경로는 점선, 미선택 후보는
옅은 점선이다. 실제 처리 중에만 흐름을 움직이고 reduced-motion에서는 정지한다.
카드 안에 현재 서비스·상태, 제어기 결정 당시 순위·검증 값, 노드 지표를 배치한다.
노드별 시작·제거는 카드 바로 아래 44px 버튼 2열이다. 단계 strip은 현재 확인된 단계만
강조하며 가상의 진행률을 만들지 않는다. 전체 클러스터 표는 아래 disclosure로 접는다.
