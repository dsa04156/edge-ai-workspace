...
### AI 실행 지도 온도
노드 사용률 아래에 CPU·GPU·시스템 온도를 같은 3열로 표시한다. 1자리 소수와 °C, 기존 muted/ink 토큰을 재사용한다. 미수집과 오래된 관측(—)을 구분하고 ACPI 센서를 CPU로 표시하지 않는다. 온도 경고 임계값은 임의로 만들지 않는다.

### 순서가 있는 AI 증강 지도
명시적인 단계 계약이 있으면 desktop에서 Nano → Orin → Spark처럼 후보를 가로로 배열하고 600px 이하에서는 세로로 전환한다. 화살표는 증강 순서, 노드 강조는 현재 요청 경로다. 각 카드에 단계·이름·hostname·검증 운용점·사용률·온도를 함께 둔다. 미관측 단계도 순서에서 제거하지 않는다.
